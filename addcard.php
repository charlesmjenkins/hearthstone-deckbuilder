<?php
	...
	// Description: This page adds a card to a user's deck
	...
	// Check that the user isn't trying to insert more of this card than allowed
	...
	$currentCountOfCard = mysqli_fetch_array($result);

	// Determine if card is elite
	$sql = "SELECT elite FROM card WHERE caid = '$caid'";
	$result = $mysqli->query($sql) or die('SQL syntax error: ' . mysql_error());
	$isElite = mysqli_fetch_array($result);

	// Determine how many copies of the card are in the deck already
	$sql = "SELECT COUNT(caid) as count FROM deck_card WHERE did = $did";
	$result = $mysqli->query($sql) or die('SQL syntax error: ' . mysql_error());
	$currentCountOfDeck = mysqli_fetch_array($result);

	if ($currentCountOfDeck['count'] == 30) // Decks cannot exceed 30 cards
	{
		echo "Decks cannot exceed 30 cards.";
	}
	else if($isElite['elite'] && $currentCountOfCard['count'] == 1) // Elite cards can only appear once in a deck
	{
		echo "Elite cards can only appear once in a deck.";
	}
	else if($currentCountOfCard['count'] == 2) // Normal cards can appear a max of two times in a deck
	{
		echo "You already have the max of two of those in your deck.";
	}
	else
	{
		// Add the card to the deck
		$sql = "INSERT INTO deck_card VALUES(DEFAULT, $did, '$caid')";
		$result = $mysqli->query($sql) or die('SQL syntax error: ' . mysql_error());
	}
